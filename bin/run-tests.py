...
python_test_goals = [
    "test_callgraph", "test_javaclass", "test_github_apis", "test_github_utils"
]
...
def run_individual_python_test(target_dir, test_name, python_exec, params):
    ...
    try:
        ...
        retcode = subprocess.Popen(
            [python_exec, f"{params['PREDICTIVE_TESTING_PATH']}/tests/{test_name}.py"],
...
def parse_opts():
    ...
    parser.add_argument(
        "--root-path", type=str, required=True,
        help="An absolute path of Python scripts to test"
    )
    parser.add_argument(
